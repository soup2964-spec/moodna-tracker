export type Marketplace =
  | "amazon"
  | "walmart"
  | "ebay"
  | "etsy"
  | "aliexpress"
  | "shopify"
  | "web"
  | "reddit"
  | "telegram"
  | "twitter"
  | "discord"
  | "kemono"
  | "bunkr"
  | "simpcity"
  | "thothub"

export type ClaimType = "copyright" | "trademark" | "counterfeit"
export type ScanJobStatus = "queued" | "running" | "completed" | "failed"
export type ScanResultStatus =
  | "new"
  | "reviewing"
  | "approved"
  | "rejected"
  | "takedown_requested"
  | "removed"
  | "reappeared"
export type AlertStatus = "unread" | "read" | "actioned"
export type SuppressionReason =
  | "false_positive"
  | "takedown_pending"
  | "takedown_submitted"
  | "resolved_removed"
  | "manual"

export type SuppressedListing = {
  id: string
  brandProfileId: string
  listingFingerprint: string
  listingUrl: string
  listingTitle?: string
  sellerName?: string
  reason: SuppressionReason
  monitorForReturn: boolean
  sourceScanResultId?: string
  notes?: string
  createdAt: string
  liftedAt?: string
}
export type TakedownStatus =
  | "draft"
  | "awaiting_owner_approval"
  | "approved"
  | "submitted"
  | "removed"
  | "rejected"

export type BrandProfile = {
  id: string
  organizationId: string
  websiteUrl: string
  brandName: string
  ownerName: string
  ownerEmail: string
  authorizedAgent: string
  notes: string
  createdAt: string
  updatedAt: string
}

export type IpAsset = {
  id: string
  brandProfileId: string
  type: "website" | "logo" | "product_url" | "product_image" | "product_description" | "page_content" | "design_palette" | "copyright_text" | "trademark"
  value: string
  sourceUrl?: string
  createdAt: string
}

export type ScanFrequency = "once" | "daily" | "twice_daily" | "weekly"
export type ScanRunSlot = "am" | "pm" | "manual"

export type ScanSchedule = {
  id: string
  brandProfileId: string
  enabled: boolean
  frequency: ScanFrequency
  amMarketplaces: Marketplace[]
  pmMarketplaces: Marketplace[]
  keywords: string[]
  timezone: string
  amRunAt: string
  pmRunAt: string
  staggerMinutes: number
  lastAmRunAt?: string
  lastPmRunAt?: string
  createdAt: string
  updatedAt: string
}

export type ScanJob = {
  id: string
  brandProfileId: string
  scheduleId?: string
  runSlot?: ScanRunSlot
  marketplaces: Marketplace[]
  keywords: string[]
  status: ScanJobStatus
  frequency: ScanFrequency
  createdAt: string
  completedAt?: string
  errorMessage?: string
}

export type ScanResult = {
  id: string
  scanJobId: string
  brandProfileId: string
  marketplace: Marketplace
  sellerName: string
  listingTitle: string
  listingUrl: string
  confidence: number
  matchReason: string
  status: ScanResultStatus
  evidenceUrls: string[]
  createdAt: string
}

export type Alert = {
  id: string
  scanResultId: string
  brandProfileId: string
  title: string
  message: string
  status: AlertStatus
  createdAt: string
}

export type TakedownRequest = {
  id: string
  scanResultId: string
  brandProfileId: string
  claimType: ClaimType
  status: TakedownStatus
  ownerAttestation: boolean
  dmcaStatement: string
  submittedTo?: Marketplace
  submittedAt?: string
  createdAt: string
}

export type DmcaSubmission = {
  id: string
  takedownRequestId: string
  marketplace: Marketplace
  submissionPayload: Record<string, unknown>
  responsePayload?: Record<string, unknown>
  externalCaseId?: string
  status: TakedownStatus
  createdAt: string
}

export type TrackerState = {
  brandProfiles: BrandProfile[]
  ipAssets: IpAsset[]
  scanSchedules: ScanSchedule[]
  scanJobs: ScanJob[]
  scanResults: ScanResult[]
  alerts: Alert[]
  takedownRequests: TakedownRequest[]
  dmcaSubmissions: DmcaSubmission[]
  suppressedListings: SuppressedListing[]
}

export type ScanDiscoveryMethod = "text_search" | "public_www" | "reverse_image" | "dropshipper_search"

export type ScanCandidate = Omit<
  ScanResult,
  "id" | "scanJobId" | "brandProfileId" | "createdAt" | "status"
> & {
  snippet?: string
  discoveryMethod?: ScanDiscoveryMethod
  sourceImageUrl?: string
}
